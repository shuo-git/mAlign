...
def train(args):
    ...
    if args.sharegpt_en_dataset is not None:
        dataset_dict['en'] = load_sharegpt_pro_data(args.sharegpt_en_dataset, "en-original")
        bmt.print_rank("en-original training instance number:", len(dataset_dict['en']))
    if args.sharegpt_zh_dataset is not None:
        dataset_dict['zh'] = load_sharegpt_data(args.sharegpt_zh_dataset, "zh")
        bmt.print_rank("zh-original training instance number:", len(dataset_dict['zh']))
    if args.sharegpt_q_switch_dataset is not None:
        dataset_dict['qs'] = load_sharegpt_pro_data(args.sharegpt_q_switch_dataset, "question-switching")
        bmt.print_rank("question-switching training instance number:", len(dataset_dict['qs']))
    
    total_training_instances = 0
    for di in args.dataset_schedule:
        total_training_instances += len(dataset_dict[di])
    
    args.train_iters = int(total_training_instances / (args.batch_size_per_device * bmt.world_size())) + 1
    ...
    for epoch, cur_dataset_name in enumerate(args.dataset_schedule):
        cur_dataset = dataset_dict[cur_dataset_name]
        indices = torch.randperm(len(cur_dataset))
        dataset = [cur_dataset[i] for i in indices]
        ...
        for step, inputs in enumerate(dataloader):
            ...
            with bmt.inspect.inspect_tensor() as inspector:
                ...
                if cur_dataset_name == 'qs':
                    teacher_labels = []
                    teacher_logits = []
                    for data_id in ids:
                        teacher_data = torch.load(args.sharegpt_en_logits_path + '/' + data_id + '.pt')
                        teacher_labels.append(teacher_data["label"])
                        teacher_logits.append(teacher_data["logits"])
                    teacher_labels = torch.stack(teacher_labels).cuda()
                    teacher_logits = torch.stack(teacher_logits).cuda()
                    flatten_t_logits = teacher_logits[teacher_labels > 0]
                    flatten_s_logits = logits[labels > 0]
                    assert (teacher_labels[teacher_labels > 0] == labels[labels > 0]).all().item()
                    kd_loss = torch.nn.KLDivLoss(reduction='batchmean')(
                        F.log_softmax(flatten_s_logits / args.kd_temperature, dim=-1),
                        F.softmax(flatten_t_logits / args.kd_temperature, dim=-1)
                    )
                    loss = ntp_loss + kd_loss
                    global_loss = bmt.sum_loss(loss).item()
                    ntp_loss_log = bmt.sum_loss(ntp_loss).item()
                    kd_loss_log = bmt.sum_loss(kd_loss).item()
                else:
                    loss = ntp_loss
                    global_loss = bmt.sum_loss(loss).item()
                    ntp_loss_log = bmt.sum_loss(ntp_loss).item()
            ...
            if global_step % args.logging_step == 0:
                bmt.print_rank(
                    "| Epoch: {:3d} | Iter: {:6d} | loss: {:.4f} ntp_loss: {:.4f} kd_loss: {:.4f} average_loss: {:.4f} | lr: {:.4e} | time: {:.4f} seconds | total_time_passed: {:.4f} minutes".format(
                        epoch,
                        global_step,
                        global_loss,
                        ntp_loss_log,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--sharegpt_en_logits_path", default=None, type=str)
    parser.add_argument("--dataset_schedule", nargs='+', default=None, required=True)
